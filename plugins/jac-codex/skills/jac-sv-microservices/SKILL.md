...
description: Splitting a Jac backend into microservices with `sv import` - compile-time HTTP clients, provider discovery environment variables, auto-spawn, remote walkers, boundary types, streaming, and gateway mode. Load when one server module calls another deployed service.
---
