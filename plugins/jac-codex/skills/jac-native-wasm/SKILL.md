...
description: "Run native-compiled Jac in the browser as WebAssembly using the client-to-native import edge, app FFI, i64 marshalling, wasm imports, and `jac nacompile --target wasm32`. Load for games, simulations, and client-side native compute. Pair with `jac-cl-components` and `jac-native`."
---
...
```jac
# host.cl.jac (or any client module)
na import from .kernel { count_primes }
...
- **Binding**: each imported name becomes a generated async stub
  ...
  `sv import` endpoint calls. `__jac_glob_init()` and BigInt marshalling of
