...
description: Memory management on the native pathway - the emit-time `--gc` modes (cycles/rc/none), the opt-in ownership and borrow checker, first-class `Region` arenas, zero-RC enforced builds, and verification. Load for E13xx/E14xx diagnostics, W1310, native leaks, refcount churn, or RC-free binaries. For the native subset itself see `jac-native`.
---
